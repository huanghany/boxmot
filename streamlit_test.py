...
def main(video_path=None):
    ...
    # 日期选择模块
    selected_date = st.sidebar.date_input("选择日期", value=datetime(2024, 11, 29))
    ...
    if os.path.exists(date_folder):
        ...
        # 如果有子文件夹，选择子文件夹
        if subfolders:
            selected_subfolder = st.sidebar.selectbox("选择子文件夹", subfolders)
        ...
        selected_video_name = st.sidebar.selectbox("选择视频", video_names_filtered)  # 选择视频名
        video_path = video_paths_filtered[
            video_names_filtered.index(selected_video_name)] if video_names_filtered else None

        if video_path:
            result_video_path = get_result_video_path(selected_video_name, Path(video_path).parent)
