...
class Tracker:
    ...
    def update(self, detections):  # 关联轨迹与det
        ...
        for detection_idx in unmatched_detections:  # 没匹配上的det
            self._initiate_track(detections[detection_idx])  # 将没匹配上的det初始化（新建一个track）
        self.tracks = [t for t in self.tracks if not t.is_deleted()]

        # Update distance metric. 更新距离指标
        active_targets = [t.id for t in self.tracks if t.is_confirmed()]  # 提取匹配上的轨迹id
        features, targets = [], []
    ...
    def _match(self, detections):  # 进行匹配
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):  # reid+门控
        ...
        unconfirmed_tracks = [i for i, t in enumerate(self.tracks) if not t.is_confirmed()]  # 没匹配过的轨迹（新轨迹）
        ...
        matches_b, unmatched_tracks_b, unmatched_detections = linear_assignment.min_cost_matching(
            iou_matching.iou_cost,  # track的预测框与det预测框距离
            self.max_iou_dist,  # 0.7
            self.tracks,  # 所有轨迹
            detections,  # 所有det
            iou_track_candidates,  # 新轨迹 + reid没匹配上的轨迹中上一帧匹配上的轨迹
            unmatched_detections,  # 剩余det
        )  # iou关联
